...
function universal_theme_widgets_init() {
	register_sidebar(
		array(
			...
			'after_title'   => '<div class="links-container">',
		)
	);
}
...
class Downloader_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e( 'Ссылка на файлы:' ); ?></label> 
...
class Tags_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// __construct( $id_base, $name, $widget_options = array(), $control_options = array() )
		parent::__construct(
			'tars_widget', // ID виджета, если не указать (оставить ''), то ID будет равен названию класса в нижнем регистре: downloader_widget
			'Облако меток',
			array( 'description' => 'Облако меток', 'classname' => 'widget-tags', )
		);

		// скрипты/стили виджета, только если он активен
		if ( is_active_widget( false, false, $this->id_base ) || is_customize_preview() ) {
			add_action('wp_enqueue_scripts', array( $this, 'add_tags_widget_scripts' ));
			add_action('wp_head', array( $this, 'add_tags_widget_style' ) );
		}
	}
		/**
	 * Вывод виджета во Фронт-энде
...
class Social_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $link1 ) ) {
			echo '<a target="_black" class="widget-link" href = "' . $link1 . '">
			<img class="widget-link-icon" src="'.  get_template_directory_uri(  ) . '/assets/images/twitter.svg" >
			</a>';
		}
		if ( ! empty( $link2 ) ) {
			echo '<a target="_black" class="widget-link" href = "' . $link2 . '">
	...
	function form( $instance ) {
		...
		// $description = @ $instance['description'] ?: 'Описание';
		$link = @ $instance['link'] ?: 'http://google.ru/';
		$link1 = @ $instance['link1'] ?: 'http://google.com/';
		$link2 = @ $instance['link2'] ?: 'http://google.ua/';
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Заголовок:' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e( 'Ссылка на соцсеть:' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'link' ); ?>" name="<?php echo $this->get_field_name( 'link' ); ?>" type="text" value="<?php echo esc_attr( $link ); ?>">
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'link1' ); ?>"><?php _e( 'Ссылка на соцсеть:' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link2' ); ?>"><?php _e( 'Ссылка на соцсеть:' ); ?></label> 
...
//регистрация Tags_Widget в WordPress
